"""veldra — thin CLI over the same HTTP/SSE endpoints the web app uses.

    veldra kb add <file>
    veldra build "answer from my docs with citations"
    veldra ask "what does section 3 say?"   [--agent <id>]
    veldra agents
    veldra selfmod <agent_id> "make answers more concise" [--apply]
"""
...
app = typer.Typer(add_completion=False, help="Veldra CLI — talk an agent into existence.")
console = Console()

STATE_DIR = Path(".veldra")
LAST_AGENT = STATE_DIR / "last_agent"


def base_url() -> str:
    return os.getenv("VELDRA_API_BASE_URL", "http://localhost:8000").rstrip("/")
...
def kb(action: str, path: str) -> None:
    """Knowledge base ops. Currently: `veldra kb add <file>`."""
    if action != "add":
...
def build(request: str, key: str = typer.Option(None, help="idempotency key")) -> None:
    ...
    if agent_id:
        ...
        console.print("\n[dim](saved as default agent; use `veldra ask`)[/]")
...
def ask(question: str, agent: str = typer.Option(None, help="agent id (defaults to last built)")) -> None:
    ...
    if not agent_id:
        console.print("[red]no agent specified and none saved — run `veldra build` first[/]")
        raise typer.Exit(1)
    citations: list[dict] = []
    console.print()
