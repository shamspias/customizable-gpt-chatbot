...
class Settings(BaseSettings):
    ...
    # ── LLM ──
    llm_provider: str = Field(default="anthropic", alias="LOOM_LLM_PROVIDER")  # anthropic | ollama
    ...
    embed_provider: Literal["auto", "openai", "ollama"] = Field(
        default="auto", alias="LOOM_EMBED_PROVIDER"
    )
    openai_embed_model: str = Field(default="text-embedding-3-small", alias="LOOM_OPENAI_EMBED_MODEL")
    ollama_embed_model: str = Field(default="nomic-embed-text", alias="LOOM_OLLAMA_EMBED_MODEL")
    ollama_base_url: str = Field(default="http://localhost:11434", alias="LOOM_OLLAMA_BASE_URL")
    embed_dim: int = Field(default=1536, alias="LOOM_EMBED_DIM")
    ...
    s3_access_key: str = Field(default="loom", alias="S3_ACCESS_KEY")
    s3_secret_key: str = Field(default="loomsecret", alias="S3_SECRET_KEY")
    s3_bucket: str = Field(default="loom-documents", alias="S3_BUCKET")
    s3_region: str = Field(default="us-east-1", alias="S3_REGION")

    # ── app ──
    env: str = Field(default="local", alias="LOOM_ENV")
    api_host: str = Field(default="0.0.0.0", alias="LOOM_API_HOST")
    api_port: int = Field(default=8000, alias="LOOM_API_PORT")
    api_base_url: str = Field(default="http://localhost:8000", alias="LOOM_API_BASE_URL")
    default_tenant_slug: str = Field(default="default", alias="LOOM_DEFAULT_TENANT")
    log_level: str = Field(default="INFO", alias="LOOM_LOG_LEVEL")

    # ── observability ──
    otel_service_name: str = Field(default="loom", alias="OTEL_SERVICE_NAME")
